...
@withTitle
class TenancyTileNew extends React.Component {
	state = {
		form: {
			tile_name: '',
			tile_description: '',
	...
	__onSubmitTiles = async () => {
		...
		try {
			...
			if (image) {
				...
				image_tile.append('file', image, image ? image.name : '')
			...
			await Promise.all(
				files.map(async files => {
					...
					files_upload_tiles.append('file', files, files.name)
			...
			this.props.history.push(`/building/${building.id}/tenancy/${tenancy.id}/app`)
		} catch (e) {
			console.log('err', e)
		}
	}
										...
										<Button
											...
											onClick={() =>
												this.props.history.push(
													`/building/${building.id}/tenancy/${tenancy.id}/app`
												)
											}
										>
											CANCEL
